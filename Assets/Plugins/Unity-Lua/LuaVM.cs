...
public class LuaVM
{
    ...
    public Table AddGlobalTable(string tableName)
    {
        ...
        {
            Debug.LogError($"Failed to add global Lua table {tableName}");
        }
    ...
    public bool SetGlobal(string key, object value)
    {
        ...
        {
            Debug.LogError($"Lua SetGlobal error: {ex.DecoratedMessage}");
        }
        return didSet;
    }
    ...
    public DynValue GetGlobal(string key)
    {
        ...
        {
            Debug.LogError($"Failed to get Lua global {key}");
        }
    ...
    public DynValue GetGlobal(params object[] keys)
    {
        ...
        {
            Debug.LogErrorFormat("Failed to get Lua global at '{0}'", 
    ...
    public DynValue ExecuteScript(string filePath)
    {
        ...
        {
            Debug.LogErrorFormat("Lua ExecuteScript error: {0}", ex.DecoratedMessage);
        }
        catch (Exception ex)
        {
            Debug.LogErrorFormat("System ExecuteScript error: {0}", ex.Message);
        }
    ...
    public DynValue Call(DynValue luaFunc, params object[] args)
    {
        ...
        if (luaFunc.IsNotNil() && luaFunc.Type == DataType.Function)
        {
            ...
            {
                Debug.LogErrorFormat("Lua Call error: {0}", ex.DecoratedMessage);
            }
        }
        else
        {
            Debug.LogErrorFormat("Invalid lua function passed to LuaVM::Call");
        }
    ...
    /// <summary>
    /// Check if a lua function exists via name
    /// </summary>
    /// <param name="functionName"></param>
    /// <returns></returns>
    public bool DoesCallExist(string functionName)
    {
    ...
    public DynValue Call(string functionName, params object[] args)
    {
        ...
        if (!string.IsNullOrEmpty(functionName))
        {
            ...
            if (func.Type == DataType.Function)
            {
                ...
                {
                    Debug.LogErrorFormat("Lua error calling function {0}: {1}", functionName, ex.DecoratedMessage);
                }
            }
            else
            {
                Debug.LogErrorFormat("Failed to find lua function '{0}'", functionName);
            }
        }
        return result;
    }
